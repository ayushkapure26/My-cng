...
class CngRepository(
    ...
    suspend fun reportLivePumpStatus(
        ...
        try {
            val syncResult = firestoreSyncRepository?.pushLivePumpStatus(
                pumpId = pumpId,
            ...
            cloudSynced = syncResult?.isSuccess == true
        } catch (_: Exception) {
            cloudSynced = false
        }
        ...
        pump?.let {
            val updated = it.copy(
                ...
                reportedByDriver = if (reporterName.isNotBlank()) "Driver $reporterName" else "Community Report",
                dataSourceType = if (cloudSynced) "Community Report (Shared)" else "Local Report (Device Only)",
                lastUpdatedTime = System.currentTimeMillis()
            )
            pumpDao.updatePump(updated)
        }
    ...
    suspend fun saveRefill(
        ...
        if (prevRefill != null && refill.odometer > prevRefill.odometer) {
            ...
            // Strict full-to-full calculation: calculate mileage only across valid comparable full tank refills
            if (refill.isFullRefill && prevRefill.isFullRefill && refill.quantityKg > 0) {
